...
layui.use(['form', 'jquery', 'laydate', 'layer', 'laypage', 'dialog',   'element'], function() {
	...
	$('.addBtn').click(function() {
		...
		parent.page("菜单添加", url + "?id=" + id, iframeObj, w = "700px", h = "520px");
		return false;
	}).mouseenter(function() {
	...
	$('#table-list,.tool-btn').on('click', '.go-btn', function() {
		var url=$(this).attr('data-url');
		var id = $(this).attr('data-id');
		window.location.href=url.substr(0,url.indexOf('.'))+"?id="+id;
		return false;
	});
	//编辑栏目
	$('#table-list').on('click', '.edit-btn', function() {
		...
		//将iframeObj传递给父级窗口
		parent.page("菜单编辑", url + "?id=" + id, iframeObj, w = "700px", h = "520px");
		return false;
	})
});
